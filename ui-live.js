...
/* app.js used to call scrollIntoView automatically after login/filter changes.
   Suppress only that automatic method; the red button uses window.scrollTo. */
...
document.addEventListener("input",event=>{
  ...
  if(container&&opponent){
    ...
    const chip=[...container.querySelectorAll(".missingGame")].find(el=>el.textContent.trim()===oldLabel);
...
function opponentFromGame(game){
  if(!game)return"";
  const teams=[...game.querySelectorAll(".duel .club span")].map(el=>el.textContent.trim()).filter(Boolean);
