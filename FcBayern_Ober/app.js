...
const MAX_TICKETS=2;
const SLOTS={p1:"122/19/19",p2:"122/19/20"};
const SLOT_ORDER=["p1","p2"];
...
const app=$("app"),q=$("q"),sv=$("sv"),s1=$("s1"),s2=$("s2"),so=$("so"),sp1=$("sp1"),sp2=$("sp2"),missing1=$("missing1"),missing2=$("missing2"),loginScreen=$("loginScreen"),loginForm=$("loginForm"),loginUser=$("loginUser"),loginPassword=$("loginPassword"),loginError=$("loginError"),userbar=$("userbar"),logoutBtn=$("logoutBtn"),syncStatus=$("syncStatus"),lastFixtureSync=$("lastFixtureSync"),greeting=$("greeting"),onlineUsers=$("onlineUsers"),presenceDropdown=$("presenceDropdown"),paypalDialog=$("paypalDialog"),paypalPerson=$("paypalPerson"),paypalMatch=$("paypalMatch"),paypalAmount=$("paypalAmount"),paypalPreview=$("paypalPreview"),paypalShare=$("paypalShare"),paypalCopy=$("paypalCopy"),paypalStatus=$("paypalStatus");
...
function slotLabel(slot){return SLOTS[slot]||slot}
function personName(id,slot){const value=String(S.names?.[id]?.[slot]||"").trim();return value||slotLabel(slot)}

async function logLogin(){if(!currentActor)return;const {error}=await sb.rpc("log_login",{p_actor_name:currentActor});if(error)console.error("Login-History:",error)}
async function logPaypal(action,data){
  const {error}=await sb.from("history_log").insert({actor_name:currentActor||"Admin",entity_type:"paypal",entity_id:data.m.id,before_data:{},after_data:{action,person:data.person,ticket:slotLabel(data.slot),opponent:data.m.o,match_label:data.matchLabel,amount:data.amount,currency:"EUR",paypal_me:"ChristianReinheimer",variant:VARIANT}});
...
async function loadRemoteState(){const {data,error}=await sb.from("season_state").select("data").eq("season",STATE_SEASON).single();if(error)throw new Error("Ober-Daten konnten nicht geladen werden: "+(error.message||error.code));S=normalizeState(data.data);render()}
...
async function saveRemoteState(){if(!currentUser)return;const {error}=await sb.from("season_state").update({data:JSON.parse(JSON.stringify(S)),updated_by:currentActor||"Admin",updated_at:new Date().toISOString()}).eq("season",STATE_SEASON);setSync(error?"Fehler beim Speichern":"gespeichert",!!error)}

function subscribeRealtime(){
  if(realtimeChannel)sb.removeChannel(realtimeChannel);
  realtimeChannel=sb.channel("fcb-ober-season-2026-27").on("postgres_changes",{event:"UPDATE",schema:"public",table:"season_state",filter:`season=eq.${STATE_SEASON}`},payload=>{if(!payload.new?.data)return;loadingRemote=true;S=normalizeState(payload.new.data);render();loadingRemote=false;setSync("synchron")}).on("postgres_changes",{event:"*",schema:"public",table:"match_overrides",filter:`season=eq.${FIXTURE_SEASON}`},async()=>{await loadRemoteMatches();await loadLastFixtureSync();render();setSync("Termine aktualisiert")}).subscribe();
}
function renderPresence(){
  if(!onlineUsers||!presenceChannel)return;
  const actors=[...new Set(Object.values(presenceChannel.presenceState()).flat().map(p=>p?.actor).filter(a=>ACTOR_ORDER.includes(a)))].sort((a,b)=>ACTOR_ORDER.indexOf(a)-ACTOR_ORDER.indexOf(b));
...
function visible(){const z=q.value.trim().toLowerCase();return M.filter(m=>{const assigned=S.assignments[m.id]||[];let ok=false;if(F==="all")ok=noAway(m);else if(F==="open")ok=rel(m)&&assigned.length<MAX_TICKETS;else if(F==="bl")ok=m.c==="bl"&&m.h===true;else if(F==="dfb"||F==="cl")ok=m.c===F&&noAway(m);else ok=rel(m);return ok&&(!z||[m.l,m.o,m.p].join(" ").toLowerCase().includes(z))})}
...
function scrollToCurrentOrNext(matches){const next=nextVisibleMatch(matches);if(!next)return;const el=document.getElementById(`game-${next.id}`);if(!el)return;document.querySelectorAll(".game.nextgame").forEach(x=>x.classList.remove("nextgame"));el.classList.add("nextgame");const headerHeight=document.querySelector(".topbar")?.getBoundingClientRect().height||0;const y=window.scrollY+el.getBoundingClientRect().top-headerHeight-24;setTimeout(()=>window.scrollTo({top:Math.max(0,y),behavior:"smooth"}),80)}
...
function card(m){
  ...
  const B=slot=>{const sel=x.includes(slot),blocked=full&&!sel,done=!!pay[slot],person=String(names[slot]||"");return`<div class="pickcell ${sel?"sel":blocked?"blocked":""}"><button class="choose" data-id="${m.id}" data-v="${slot}" ${blocked?"disabled":""}>${E(slotLabel(slot))}</button>${sel?`<input class="guestname ticketname" data-ticket-name="${m.id}" data-slot="${slot}" value="${E(person)}" placeholder="Name für ${E(slotLabel(slot))}">`:""}${sel&&!done?`<button class="paypalRequest" type="button" data-paypal-id="${m.id}" data-paypal-person="${slot}">PayPal anfordern</button>`:""}${sel?`<label class="paymini ${done?"done":""}"><input type="checkbox" data-pay="${m.id}" data-person="${slot}" ${done?"checked":""}> ${done?"bezahlt ✓":"bezahlt"}</label>`:""}</div>`};
...
function missingLabel(m,slot){return`${personName(m.id,slot)} – ${m.o}`}
function updatePaymentLive(slot){const items=[];for(const game of visible()){const assigned=S.assignments[game.id]||[];if(assigned.includes(slot)&&!S.paid[game.id]?.[slot])items.push(missingLabel(game,slot))}const target=slot==="p1"?missing1:missing2;if(target)target.innerHTML=paymentMissingHtml(items)}
function render(){
  ...
  const counts={p1:0,p2:0},paidCounts={p1:0,p2:0},missing={p1:[],p2:[]};let open=0;
  v.forEach(m=>{const x=S.assignments[m.id]||[],pay=S.paid[m.id]||{};if(x.length<MAX_TICKETS)open++;for(const slot of SLOT_ORDER){if(x.includes(slot)){counts[slot]++;if(pay[slot])paidCounts[slot]++;else missing[slot].push(missingLabel(m,slot))}}});
  sv.textContent=v.length;s1.textContent=counts.p1;s2.textContent=counts.p2;so.textContent=open;sp1.textContent=paidCounts.p1;sp2.textContent=paidCounts.p2;missing1.innerHTML=paymentMissingHtml(missing.p1);missing2.innerHTML=paymentMissingHtml(missing.p2);
...
function paypalData(){if(!paymentRequest)return null;const amount=parseAmount(paypalAmount?.value);if(!amount)return null;const m=M.find(x=>x.id===paymentRequest.id);if(!m)return null;const slot=paymentRequest.person,person=personName(m.id,slot),amountText=new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(amount),link=`${PAYPAL_ME}/${amount.toFixed(2)}`,date=dt(m)[0],matchLabel=`${m.l} · ${m.o}`;const message=`Hi ${person},\n\n${matchLabel}\nTicket: ${slotLabel(slot)}\nDatum: ${date}\nBetrag: ${amountText}\n\nPayPal: ${link}`;return{m,slot,person,amount,amountText,link,date,matchLabel,message}}
...
function bind(){
  document.querySelectorAll(".choose,.openpick").forEach(b=>b.onclick=()=>{const id=b.dataset.id,v=b.dataset.v;let x=[...(S.assignments[id]||[])];if(v==="open"){x=[];S.paid[id]={};delete S.names[id]}else if(x.includes(v)){x=x.filter(y=>y!==v);if(S.paid[id])delete S.paid[id][v];if(S.names[id]){delete S.names[id][v];if(!Object.keys(S.names[id]).length)delete S.names[id]}}else if(x.length<MAX_TICKETS)x.push(v);S.assignments[id]=x;queueSave();render()});
  document.querySelectorAll("[data-ticket-name]").forEach(e=>{e.onclick=ev=>ev.stopPropagation();e.oninput=()=>{const id=e.dataset.ticketName,slot=e.dataset.slot;S.names[id]||={};S.names[id][slot]=e.value;updatePaymentLive(slot)};e.onchange=()=>queueSave();e.onkeydown=ev=>{if(ev.key==="Enter"){ev.preventDefault();e.blur()}}});
...
q.oninput=render;$("currentMatchBtn")?.addEventListener("click",()=>scrollToCurrentOrNext(visible()));paypalAmount?.addEventListener("input",updatePaypalPreview);paypalCopy?.addEventListener("click",copyPaypal);paypalShare?.addEventListener("click",sharePaypal);paypalDialog?.addEventListener("close",()=>{paymentRequest=null;paypalStatus.textContent=""});
onlineUsers?.addEventListener("click",e=>{e.stopPropagation();presenceDropdown.hidden=!presenceDropdown.hidden;onlineUsers.setAttribute("aria-expanded",String(!presenceDropdown.hidden))});document.addEventListener("click",e=>{if(!e.target.closest(".presenceWrap")&&presenceDropdown){presenceDropdown.hidden=true;onlineUsers?.setAttribute("aria-expanded","false")}});

loginForm.addEventListener("submit",async e=>{e.preventDefault();loginError.textContent="";const actor=ACTORS[loginUser.value];if(!actor){loginError.textContent="Bitte einen Benutzer auswählen.";return}const {data,error}=await sb.auth.signInWithPassword({email:LOGIN_EMAIL,password:loginPassword.value});if(error){loginError.textContent="Passwort ist nicht korrekt.";return}try{currentActor=actor;sessionStorage.setItem(ACTOR_KEY,actor);showApp(data.user);await logLogin();subscribePresence();await loadRemoteMatches();await loadLastFixtureSync();await loadRemoteState();subscribeRealtime();loginPassword.value=""}catch(err){console.error(err);await sb.auth.signOut();sessionStorage.removeItem(ACTOR_KEY);showLogin(err?.message||"Saisondaten konnten nicht geladen werden.")}});
...
async function boot(){const {data:{session}}=await sb.auth.getSession();if(!session?.user){showLogin();return}currentActor=sessionStorage.getItem(ACTOR_KEY);if(!ACTOR_ORDER.includes(currentActor)){await sb.auth.signOut();showLogin();return}try{showApp(session.user);subscribePresence();await loadRemoteMatches();await loadLastFixtureSync();await loadRemoteState();subscribeRealtime()}catch(err){console.error(err);await sb.auth.signOut();sessionStorage.removeItem(ACTOR_KEY);showLogin(err?.message||"Saisondaten konnten nicht geladen werden.")}}
boot();