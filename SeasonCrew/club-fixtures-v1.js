(()=>{
  ...
  function renderRows(rows){const list=$('fixtureList');if(!list)return;list.innerHTML=rows.length?rows.map(f=>`<div class="ticketSettingRow"><div><b>${esc(f.label)}</b><small class="fixtureSettingMeta"><span>${esc(f.competition_name||competitionMeta(f.competition_key).name)} · ${esc(f.opponent)}</span>${f.venue?`<span>${esc(f.venue)}</span>`:''}<span>${new Intl.DateTimeFormat('de-DE').format(new Date(`${f.date_start}T12:00:00`))}${f.time_text?` · ${esc(String(f.time_text).slice(0,5))} Uhr`:''}</span><span>${esc(money(f.price_override))}</span></small></div><button class="dangerButton" type="button" data-delete-fixture="${esc(f.fixture_id)}">Löschen</button></div>`).join(''):'<div class="loadingCard">Noch keine eigenen Heimspiele angelegt.</div>';list.querySelectorAll('[data-delete-fixture]').forEach(b=>b.addEventListener('click',()=>deleteFixture(b.dataset.deleteFixture)))}
  async function addFixture(){
    if(!isManager())return;const c=client(),group=gid(),label=$('fixtureLabel')?.value.trim(),opponent=$('fixtureOpponent')?.value.trim(),venue=$('fixtureVenue')?.value.trim(),date=$('fixtureDate')?.value,time=$('fixtureTime')?.value,price=parseMoney($('fixturePrice')?.value),competition=$('fixtureCompetition')?.value||'other',competitionInfo=competitionMeta(competition);
    if(!group||!label||!opponent||!date){toast('Bitte Wettbewerb/Bezeichnung, Gegner und Datum angeben');return}
    const {data:{session}}=await c.auth.getSession();const {error}=await c.from('sc_fixtures').insert({group_id:group,competition_key:competitionInfo.key,competition_name:competitionInfo.name,label,date_start:date,date_end:date,time_text:time||null,opponent,venue:venue||null,is_home:true,price_override:price,source:'manual',created_by:session?.user?.id||null});
    if(error){toast('Heimspiel konnte nicht gespeichert werden');console.error(error);return}
    ['fixtureLabel','fixtureOpponent','fixtureVenue','fixtureDate','fixtureTime','fixturePrice'].forEach(id=>{if($(id))$(id).value=''});toast('Heimspiel hinzugefügt');window.dispatchEvent(new CustomEvent('seasoncrew:fixtures-updated',{detail:{groupId:group}}));schedule(30);
  }
