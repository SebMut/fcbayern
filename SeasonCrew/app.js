...
const els={
  ...
  groupSelect:$('groupSelect'),noGroups:$('noGroups'),workspace:$('workspace'),helloUser:$('helloUser'),seasonPill:$('seasonPill'),groupTitle:$('groupTitle'),clubName:$('clubName'),memberRole:$('memberRole'),copyJoinCode:$('copyJoinCode'),onlineBadge:$('onlineBadge'),syncInfo:$('syncInfo'),games:$('games'),searchInput:$('searchInput'),
...
function roleLabel(r){return r==='owner'?'Owner':r==='admin'?'Admin':'Mitglied'}
function isAdmin(){const r=memberships.get(currentGroup?.id);return r==='owner'||r==='admin'}
...
els.signupForm.addEventListener('submit',async e=>{
  e.preventDefault();setStatus(els.authStatus,'Account wird erstellt …');
  const display_name=$('signupName').value.trim();
  const {data,error}=await sb.auth.signUp({email:$('signupEmail').value.trim(),password:$('signupPassword').value,options:{data:{display_name}}});
  if(error){setStatus(els.authStatus,error.message);return}
  if(!data.session){setAuthTab('login');setStatus(els.authStatus,'Account angelegt. Bitte bestätige jetzt die E-Mail und logge dich danach ein.',true);return}
  session=data.session;user=data.user;await enterApp();
});
...
async function loadProfile(){
  const {data,error}=await sb.from('sc_profiles').select('id,display_name').eq('id',user.id).maybeSingle();
  if(error)console.error(error);
  profile=data||{id:user.id,display_name:user.email?.split('@')[0]||'Fan'};
}
...
async function loadGroups(preferId=null){
  ...
  if(!ids.length){groups=[];currentGroup=null;renderGroupSelector();els.workspace.classList.add('hidden');els.noGroups.classList.remove('hidden');return}
  const {data:gs,error}=await sb.from('sc_groups').select('*').in('id',ids).order('created_at');
  if(error){console.error(error);return}
  groups=gs||[];renderGroupSelector();
...
async function loadCurrentGroup(){
  ...
  await loadFixtures();await enrichMembers();render();setupPresence();setupRealtime();
}

async function enrichMembers(){
  const ids=members.map(m=>m.user_id);if(!ids.length)return;
  const {data}=await sb.from('sc_profiles').select('id,display_name').in('id',ids);
  const map=new Map((data||[]).map(p=>[p.id,p.display_name]));members=members.map(m=>({...m,display_name:map.get(m.user_id)||'Mitglied'}));
}
...
function render(){
  ...
  els.helloUser.textContent=`Hallo ${profile?.display_name||'Fan'}`;
  els.seasonPill.textContent=`Saison ${currentGroup.season.replace('-', ' / ')}`;
  els.groupTitle.textContent=currentGroup.name;els.clubName.textContent=currentGroup.club_name;els.memberRole.textContent=roleLabel(memberships.get(currentGroup.id));els.copyJoinCode.textContent=currentGroup.join_code;
...
async function assignTicket(fixtureId,ticketId){
  const row={group_id:currentGroup.id,fixture_id:fixtureId,ticket_id:ticketId,attendee_name:profile?.display_name||'',attendee_user_id:user.id,paid:false,amount:Number(currentGroup.default_price)||50,updated_by:user.id};
...
function renderSettings(){
  if(!currentGroup)return;$('settingsTitle').textContent=currentGroup.name;$('profileName').value=profile?.display_name||'';$('settingsGroupName').value=currentGroup.name;$('settingsPaypal').value=cleanPaypal(currentGroup.paypal_me);$('settingsPrice').value=Number(currentGroup.default_price||50).toFixed(2).replace('.',',');
  $('adminSettings').classList.toggle('hidden',!isAdmin());$('ticketSettings').classList.toggle('hidden',!isAdmin());
  ...
  $('memberList').innerHTML=members.map(m=>`<div class="memberRow"><b>${esc(m.display_name||'Mitglied')}</b><span>${roleLabel(m.role)}</span></div>`).join('');
}
...
$('saveProfileBtn').addEventListener('click',async()=>{const name=$('profileName').value.trim();if(!name)return;const {error}=await sb.from('sc_profiles').update({display_name:name,updated_at:new Date().toISOString()}).eq('id',user.id);if(error){setStatus($('settingsStatus'),error.message);return}profile.display_name=name;setStatus($('settingsStatus'),'Profil gespeichert ✓',true);render()});
...
$('joinGroupForm').addEventListener('submit',async e=>{e.preventDefault();setStatus($('joinGroupStatus'),'Beitritt wird geprüft …');const {data,error}=await sb.rpc('sc_join_group',{p_code:$('joinCode').value.trim()});if(error){setStatus($('joinGroupStatus'),error.message);return}els.joinDialog.close();$('joinCode').value='';await loadGroups(data);showToast('Crew beigetreten')});
els.copyJoinCode.addEventListener('click',async()=>{await navigator.clipboard.writeText(currentGroup.join_code);showToast('Beitrittscode kopiert')});
...
async function boot(){const {data:{session:s}}=await sb.auth.getSession();session=s;user=s?.user||null;if(!user){document.body.classList.add('auth-locked');els.authScreen.classList.remove('hidden');return}await enterApp()}
boot();
