...
const VARIANT="tom";
const HISTORY_RESET_AT=Date.parse("2026-08-16T10:19:31.482Z");
const PERSON={t1:"Tom1",t2:"Tom2",s1:"Staudter 1",s2:"Staudter 2"};
...
function gameName(id){const m=MATCHES.get(id);if(!m)return id||"Saisonübersicht";const comp=id.startsWith("bl")?"Bundesliga":id.startsWith("dfb")?"DFB-Pokal":"Champions League";return `${comp} · ${m.opponent||"Gegner offen"}`}
function personName(state,id,slot){return String(state?.names?.[id]?.[slot]||"").trim()||PERSON[slot]||slot}
function namesFor(state,id,arr){return(arr||[]).map(slot=>`${PERSON[slot]||slot}${personName(state,id,slot)!==(PERSON[slot]||slot)?` (${personName(state,id,slot)})`:""}`).join(", ")||"Offen"}

function relevant(log){
  const created=Date.parse(log.created_at);
  if(!Number.isFinite(created)||created<HISTORY_RESET_AT)return false;
  if(log.entity_type==="season_state")return log.entity_id===STATE_SEASON;
  if(log.entity_type==="paypal")return log.after_data?.variant===VARIANT;
  if(log.entity_type==="login")return log.after_data?.variant===VARIANT||log.entity_id==="tom-session";
...
function stateChanges(log){
  ...
  const ids=new Set([
    ...
    ...Object.keys(a.names||{}),...Object.keys(b.names||{}),
  ...
  for(const id of ids){
    ...
    for(const slot of Object.keys(PERSON)){
      const oldName=String(a.names?.[id]?.[slot]||"").trim();
      const newName=String(b.names?.[id]?.[slot]||"").trim();
      if(oldName!==newName)out.push({id,text:`${PERSON[slot]} · Name: ${oldName||"–"} → ${newName||"–"}`});
    }

    const ap=(a.paid||{})[id]||{},bp=(b.paid||{})[id]||{};
    for(const slot of Object.keys(PERSON)){
      if(!!ap[slot]!==!!bp[slot])out.push({id,text:`${personName(b,id,slot)} · ${PERSON[slot]}: ${bp[slot]?"bezahlt":"nicht bezahlt"}`});
    }
...
function paypalChanges(log){
  ...
  if(b.ticket)out.push({id:log.entity_id,text:`Jahreskarte: ${b.ticket}`});
...
async function boot(){
  ...
  const [{data:logs,error},{data:matches}]=await Promise.all([
    sb.from("history_log").select("id,created_at,actor_name,entity_type,entity_id,before_data,after_data").gte("created_at",new Date(HISTORY_RESET_AT).toISOString()).order("created_at",{ascending:false}).limit(1000),
