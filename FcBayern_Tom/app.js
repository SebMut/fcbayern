...
let M=BASE_M.map(x=>({...x})),F="all",S={assignments:{},paid:{},notes:{},prices:{}},currentUser=null,currentActor=null,saveTimer=null,realtimeChannel=null,presenceChannel=null,loadingRemote=false,pendingAutoScroll=true,paymentRequest=null;

function E(x){return String(x??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}
function normalizeState(x){x=x||{};return{assignments:x.assignments||{},paid:x.paid||{},notes:x.notes||{},prices:x.prices||{}}}
...
async function logPaypal(action,data){
  const {error}=await sb.from("history_log").insert({actor_name:currentActor||"Admin",entity_type:"paypal",entity_id:data.m.id,before_data:{},after_data:{action,person:data.person,opponent:data.m.o,amount:data.amount,currency:"EUR",paypal_me:"tommaerz",variant:VARIANT}});
  if(error)console.error("PayPal-History:",error);
}
async function loadLastFixtureSync(){const {data,error}=await sb.from("fixture_sync_runs").select("finished_at").eq("status","success").not("finished_at","is",null).order("finished_at",{ascending:false}).limit(1).maybeSingle();if(error||!data?.finished_at){lastFixtureSync.textContent=error?"Letzter Spieltagssync: –":"Letzter Spieltagssync: noch nie";return}lastFixtureSync.textContent=`Letzter Spieltagssync: ${new Intl.DateTimeFormat("de-DE",{timeZone:"Europe/Berlin",day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).format(new Date(data.finished_at))}`}
async function loadRemoteState(){const {data,error}=await sb.from("season_state").select("data").eq("season",STATE_SEASON).single();if(error)throw new Error("Tom-Daten konnten nicht geladen werden: "+(error.message||error.code));S=normalizeState(data.data);render()}
async function loadRemoteMatches(){const {data,error}=await sb.from("match_overrides").select("id,start_date,end_date,kickoff_time,opponent,home,possible,active").eq("season",FIXTURE_SEASON);if(error){M=BASE_M.map(x=>({...x}));return}const byId=new Map((data||[]).map(x=>[x.id,x]));M=BASE_M.map(base=>{const x=byId.get(base.id);if(x?.active===false)return null;if(!x)return{...base};return{...base,s:x.start_date||base.s,e:x.end_date||x.start_date||base.e,t:x.kickoff_time?String(x.kickoff_time).slice(0,5):(x.start_date?"":base.t),o:x.opponent||base.o,h:x.home??base.h,pos:x.possible??base.pos}}).filter(Boolean)}
...
function subscribePresence(){if(presenceChannel)sb.removeChannel(presenceChannel);const random=crypto?.randomUUID?.()||Math.random().toString(36).slice(2);presenceChannel=sb.channel("fcb-online-tom-2026-27",{config:{presence:{key:`${currentActor}-${random}`}}}).on("presence",{event:"sync"},renderPresence).on("presence",{event:"join"},renderPresence).on("presence",{event:"leave"},renderPresence).subscribe(async status=>{if(status==="SUBSCRIBED"){await presenceChannel.track({actor:currentActor,variant:VARIANT,online_at:new Date().toISOString()});renderPresence()}})}
...
function card(m){
  const [d,sub]=dt(m),[a,b]=T(m),x=S.assignments[m.id]||[],pay=S.paid[m.id]||{},full=x.length===MAX_TICKETS,fullyPaid=full&&x.every(v=>!!pay[v]);
  const B=slot=>{const sel=x.includes(slot),blocked=full&&!sel,done=!!pay[slot];return`<div class="pickcell ${sel?"sel":blocked?"blocked":""}"><button class="choose" data-id="${m.id}" data-v="${slot}" ${blocked?"disabled":""}>${E(slotName(slot))}</button>${sel&&!done?`<button class="paypalRequest" type="button" data-paypal-id="${m.id}" data-paypal-person="${slot}">PayPal anfordern</button>`:""}${sel?`<label class="paymini ${done?"done":""}"><input type="checkbox" data-pay="${m.id}" data-person="${slot}" ${done?"checked":""}> ${done?"bezahlt ✓":"bezahlt"}</label>`:""}</div>`};
...
function render(){
  ...
  v.forEach(m=>{const x=S.assignments[m.id]||[],pay=S.paid[m.id]||{};if(x.length<MAX_TICKETS)open++;for(const s of SLOT_ORDER){if(x.includes(s)){counts[s]++;if(pay[s])paidCounts[s]++;else missing[s].push(m.o)}}});
...
function paypalData(){if(!paymentRequest)return null;const amount=parseAmount(paypalAmount?.value);if(!amount)return null;const m=M.find(x=>x.id===paymentRequest.id);if(!m)return null;const person=slotName(paymentRequest.person),amountText=new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(amount),link=`${PAYPAL_ME}/${amount.toFixed(2)}`,date=dt(m)[0],message=`Hi, für das Ticket ${person} bei FC Bayern – ${m.o} am ${date} sind noch ${amountText} offen. Du kannst hier per PayPal bezahlen: ${link}`;return{m,person,amount,amountText,link,message}}
...
function openPaypalRequest(id,person){const m=M.find(x=>x.id===id);if(!m)return;paymentRequest={id,person};paypalPerson.textContent=slotName(person);paypalMatch.textContent=`FC Bayern – ${m.o} · ${dt(m)[0]}`;paypalAmount.value=S.prices[id]??50;paypalStatus.textContent="";updatePaypalPreview();paypalDialog.showModal();setTimeout(()=>paypalAmount.focus(),50)}
...
function bind(){
  document.querySelectorAll(".choose,.openpick").forEach(b=>b.onclick=()=>{const id=b.dataset.id,v=b.dataset.v;let x=[...(S.assignments[id]||[])];if(v==="open"){x=[];S.paid[id]={}}else if(x.includes(v)){x=x.filter(y=>y!==v);if(S.paid[id])delete S.paid[id][v]}else if(x.length<MAX_TICKETS)x.push(v);S.assignments[id]=x;queueSave();render()});
